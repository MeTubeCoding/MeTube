...
app.post('/api/channels', async (req, res) => {
  try {
    ...
    const result = await channels.insertOne(query);

    console.log('Inserted document with _id:', result.insertedId);

    res.json({ message: 'Channel created', insertedId: result.insertedId });

  } catch (error) {
    console.error('Error inserting channel:', error);
    res.status(500).json({ message: 'Error inserting channel', error });
  } finally {
    await client.close();
  }
});
