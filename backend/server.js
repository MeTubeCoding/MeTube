...
app.post("/signup", async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ success: false, message: "Error while creating user profile" })
	}
	res.end()
})
...
app.get("/profile", async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ success: false, message: "Error while fetching user profile" })
	}
...
app.post("/dataLive", async (req, res) => {
	try {
		const database = client.db("METUBE")
